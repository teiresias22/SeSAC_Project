...
//  SeSACFriend
//
//  Created by Joonhwan Jeon on 2022/05/03.
//
...
class HomeChattingReportViewController: BaseViewController {
    ...
    let chattingReportView: ChattingRateView = {
        ...
        view.rateButton.setTitle("신고하기", for: .normal)
        view.rateTagCollectionView {
            
            view.register(ChattingRateCollectionViewCell.self, forCellWithReuseIdentifier: ChattingRateCollectionViewCell.identifier)
    ...
    override func viewDidLoad() {
        ...
        view.backgroundColor = UIColor.customBlack?.withAlphaComponent(0.5)
    ...
    @objc func onRateButtonClicked() {
        ...
        if chattingReportView.rateButton.style == .fill {
            ...
            let form = ReportOtherFrom(otheruid: UserDefaults.standard.string(forKey: UserDefault.otherUid.rawValue)!, reportedReputation: reportTagsSelected, comment: comment)
